#!/bin/bash
# Usage: dar-inc-filesystem.sh
# Purpose: Creates a incrmental DAR backup of a defined filesystem/directory
#   An existing differential backup must be created first with dar-fdiff-filesystem.sh
...
LASTBU=$(find "$BUPATH" -exec basename {} \; | sort | tail -1)
LASTBU=$(echo "$LASTBU" | cut -d'.' -f 1)
Help(){
   echo -e "Syntax: $0 -[h|v]"
   echo -e "Options:"
...
checkPrevious(){
   PREVIOUS=$(ls "$BUPATH" | grep "\-inc\-")
   if [[ -n "$PREVIOUS" ]]; then
      return 0
   else
      echo -e "Backup failed. Previous backup set not detected." >> "$LOGPATH/$LOGFILE"
      exit 1
   fi
}

darBackup(){
"$DAREXEC" -c "$BUPATH/$BUNAME-inc-$NOW" -s 654M -S 640M  -w -va -D  -M -R "$BUTGT" -an   -X "*.tmp" -X "*.temp" -X "*.cache" -X ".lock" -X ".swp" -P "tmp" -P "SteamLibrary" -P "lost+found" -A "$BUPATH/$LASTBU"
