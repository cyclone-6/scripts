#!/bin/bash
# Usage: dar-fdiff-filesystem.sh
# Purpose: Creates a first differential DAR backup of a defined filesystem/directory
#    A full backup must first be created with dar-full-filesystem.sh
...
checkFull(){
   FULL=$(ls "$BUPATH" | grep full.1.dar)
   echo "$FULL"
   if [[ -n "$FULL" ]]; then
      return 0
   else
      echo -e "Backup failed. Full backup set not detected." >> "$LOGPATH/$LOGFILE"
      exit 1
   fi
}
darBackup(){
"$DAREXEC" -c "$BUPATH/$BUNAME-inc-$NOW" -s 654M -S 640M  -w -va -D  -M -R "$BUTGT" -an   -X "*.tmp" -X "*.temp" -X "*.cache" -X ".lock" -X ".swp" -P "tmp" -P "SteamLibrary" -P "lost+found" -A "$BUPATH/$BUNAME-full"
